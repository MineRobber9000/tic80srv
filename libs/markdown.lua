...
local function fenced_codeblocks(lines)
   local function find_codeblock(lines)
      local start
      for i,line in ipairs(lines) do
         if line.type == "fence" then start = i+1 break end
      end
      if not start then return nil end

      local stop = #lines
      for i = start+1, #lines do
         if lines[i].type == "fence" then
            stop = i-1
            break
         end
      end
   ...
   local function process_codeblock(lines)
      local raw = detab(encode_code(lines[1].line))
      for i = 2,#lines do
         raw = raw .. "\n" .. detab(encode_code(lines[i].line))
      end
      return "<pre><code>" .. raw .. "\n</code></pre>"
   end
   ...
   while true do
      ...
      lines = splice(lines, start-1, stop+1, {info})
   end
end
